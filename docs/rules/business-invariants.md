...
| Delete and disable are distinct features, for both products and variants              | `products`, `product_variants`     | `is_active=false` removes it from the storefront but the manager still sees it; `deleted_at` removes it everywhere, manager included. `products` can never be hard-deleted — `product_variants` references it with `RESTRICT`, and `order_items` references variants, so the chain protecting order history is closed by FK, not just convention. `product_variants` mirrors this exactly via its own `deleted_at`: variant delete is now always a soft delete that always succeeds, replacing the old hard `DELETE` that `order_items`'s FK blocked once a variant had been sold — history survives because `order_items` already freezes `product_name`/`variant_label`/`unit_price_cents` independent of the variant row                                                                       |
...
| Cart checkout is a two-step flow: `POST /orders` then `POST /checkout/payment-intent` | `orders`, `payments`, `cart_items` | `POST /orders` freezes the cart into a `PENDING` order (item lines, promo application) with zero `payments` rows yet; `POST /checkout/payment-intent` creates the Stripe PaymentIntent for that existing order. Splitting them lets a client show an order summary before committing to pay, but means `OrderSummary.paymentMethod` and `OrderDetail.payment` are `null` until a payment attempt exists. A user may have at most one non-terminal (`PENDING`, unpaid) order at a time — `POST /orders` returns `409` if one already exists — because the cart is only cleared on webhook-confirmed payment (see the row above), so nothing else stops repeated `POST /orders` calls from each consuming a promo-code redemption slot (R5) for free                                                |
